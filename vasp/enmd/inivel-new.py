...
def initialize_velocity(poscar='POSCAR', aindices=[0,1], temperature=300):
    ...
    coords = []
    for i in range(len(aindices)//4):
        coord = []
        coord.append(aindices[4*i])
        coord.append([aindices[4*i+1],aindices[4*i+2]])
        coord.append(float(aindices[4*i+3]))
        coords.append(coord)
    ...
    for c in coords:
        ...
        vels[ia], vels[ib] = coef*va, vb

        print('Make Atom_a oriented to Atom_b, %.4f times Velocity_a ...' %coef)
