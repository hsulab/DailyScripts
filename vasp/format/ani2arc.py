...
def ani2arc(ani='OUT.ANI', frame=None):
    # select frame
    if frame:
        frame = int(frame)

    # read lattice
    lx, ly, lz, alpha, beta, gamma, trans_matrix = mat2lat()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--file", default='OUT.ANI', \
            help="VASP ANI FORMAT FILE")
    parser.add_argument("-s", "--select", help="select given frame")

    args = parser.parse_args()
    ani2arc(args.file, args.select)
