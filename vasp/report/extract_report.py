    ...
    def parse_report(self, datfile, nframe, ndrop):
        ...
        if ftype == 'US':
            ...
            for n in range(ncon):
                ...
                with open(bname+str(n+1)+'.dat', 'w') as writer:
                    writer.write(content)
...
def plot_usdata(datfile, col):
    ...
    #
    #ax2.set_title('Umbrella Sampling', fontsize=20, fontweight='bold')
    #ax2.set_xlabel('Collective Variable', fontsize=16)
    #ax2.set_ylabel('Number', fontsize=16)
    nbins = int(np.ceil((np.max(cval)-np.min(cval))/BINWIDTH)+1)

    n, bins, p = \
            ax.hist(cval, bins=nbins, color='g', alpha=0.2, orientation='horizontal')
    centres = []
