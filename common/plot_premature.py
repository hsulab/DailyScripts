...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--columns', nargs='*', type=int, \
            default=[1], help='data column index in file')
    parser.add_argument('-v', '--values', nargs='*', type=float, \
            default=[0.7], help='expected values for the data')

    args = parser.parse_args()

    assert len(args.columns) == len(args.values)

    # read data
    data = read_arrays(args.datafile)

    for idx, row in enumerate(data):
        if ( row[args.columns] > args.values).all():
            print(idx, row[args.columns])
    #else:
    #    print('not cross the barrier')
